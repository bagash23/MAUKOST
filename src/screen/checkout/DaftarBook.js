import { Dimensions, StatusBar, StyleSheet, Text, View, Image, TouchableOpacity, ScrollView, Modal, ActivityIndicator } from 'react-native'
...
const DaftarBook = () => {
    ...
    if ( rooms > Daftar.jumlahKamar ) {
        toastMessage({
            type: 'error',
            title: 'Jumlah Kamar',
            message: 'Jumlah kamar tidak boleh lebih besar dari jumlah kamar yang tersedia',
        });
    }
    ...
    const handlePress = async () => {
        try {
            if ( tanggalKunjungan.getDate() === new Date().getDate() && tanggalKunjungan.getMonth() === new Date().getMonth() && tanggalKunjungan.getFullYear() === new Date().getFullYear() ) {
                toastMessage({
                    type: 'error',
        ...
        <View style = {styles.container} >
            ...
            <ScrollView showsVerticalScrollIndicator = {false} >
                <View style = {styles.headerContent} >
                    <Text style = {styles.titleHeader} adjustsFontSizeToFit  >Hello {currentUser.displayName}</Text>
                    <Text style = {styles.deskHeader} adjustsFontSizeToFit >Kamu akan Booking {Daftar.nameKontrakan}</Text>
                </View>
                <View style = {styles.bodyDaftar} >
                    <Text style = {styles.titleDaftar} adjustsFontSizeToFit >Detail Booking</Text> 
                    <View style = {styles.infoKontrakan} >
                        <Image source = {{uri: Daftar.thumbnileImage}} style = {styles.imageKontrakan} />
                        <View style = {{
                            flexDirection: 'column',
                            marginLeft: 10,
                        }} >
                    ...
                    <View style = {styles.bodyInputan} >
                        ...
                        >                            
                            <Text style = {styles.textInputan} >{tanggalKunjungan.toDateString()}</Text>
            ...
            <View style = {styles.bottomCheck} >
                <View style = {styles.bodyPrice} >
                    {hargaSemua ? (
                        ...
                        <Text style = {styles.harga} >Rp. {Daftar.hargaKontrakan.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</Text>
                    )}
                    {/* <Text style = {styles.harga} >Rp. {hargaSemua.toFixed().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</Text> */}
                ...
                <TouchableOpacity 
                    ...
                    onPress = {handlePress} 
                    disabled = {rooms > Daftar.jumlahKamar || !rooms || !tanggalKunjungan || !guest} 
                >
                    <Text style = {{
