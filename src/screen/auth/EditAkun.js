...
const EditAkun = () => {
  ...
  const handleAvatar = async () => {
    ...
    if (!result.didCancel) {
      setUbahAvatar(
        result.assets.filter(asset => asset.fileName === asset.fileName),
      );
    }
  };
  ...
  const handlePressUpdate = async () => {
    ...
    if (ubahAvatar) {
      const {url} = await uploadImage(
        ubahAvatar[0].uri,
        `images/avatars/${currentUser.uid}`,
        'profilePicture',
      );
      photoURL = url;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.bgAvatar} onPress={handleAvatar}>
        <Image
          source={
            ubahAvatar[0]
              ? {uri: ubahAvatar[0].uri}
