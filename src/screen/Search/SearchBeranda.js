import { Dimensions, FlatList, Image, Modal, StyleSheet, Text, TouchableOpacity, View, Pressable, ScrollView } from 'react-native'
...
const SearchBeranda = () => {
    ...
    const DataKontrakan = query(collection(db, "Kontrakan"), where("user", "!=", currentUser.uid))
    useEffect(() => {
    ...
    const dataKota = selectedItem.filter(item => item.kota).map(item => item.kota)
    ...
    return (
        <View style = {styles.container} >
            <View style = {styles.header} >
                ...
                <Text style = {styles.textHeader} >MAUKOST</Text>
            </View>
            <ScrollView showsVerticalScrollIndicator = {false} >
                <TouchableOpacity style = {styles.contentLokasi} onPress = {() => setOpenMoodal(true)} >
                    {lokasi ? (
                ...
                <View style = {styles.card} >
                    <Text style = {styles.textCard} >Sekarang masi jamannya cari kontrakan keliling?</Text>
                    <Text style = {styles.textDekCard} >Gausah khawatir kami menyediakan bagi kalian yang ingin menyewa tempat tinggal sementara banyak berbagai pilihan untuk mencari tempat tinggal yang nyaman bagi anda</Text>
                </View>
                <FlatList
                        ...
                        return (
                            <Pressable key={index} style = {styles.bodyListLainnya} onPress = {() => navigation.navigate("TempatDetail", {item})} >
                                ...
                                <View style = {{
                                    ...
                                    {item.jumlahKamarTerisi == item.jumlahKamar ? (
                                    <View style = {{
                                    flexDirection: "row",
                                    alignItems: 'center',
                                    }} >
                                    ...
                                    <Text style = {styles.textListLainnya} >{item.nameKontrakan}</Text>
                                    <View style = {{flexDirection: 'row', alignItems: 'center'}} >
                                    ...
                                    <View style = {styles.bodyPrice} >
                                        ...
                                        <Text style = {styles.harga} >Rp. {item.hargaKontrakan.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</Text>
                                    </View>
                                </View>
                            </Pressable>
                        )
            ...
            {openModal ? (
                <Modal animationType='slide'>
                    ...
                    <FlatList                        
                        ...
                        renderItem={({ item }) => {
                            return (
                                <TouchableOpacity onPress={() => setLokasi(item) === setOpenMoodal(false) }>                                    
                                    <View style = {{
                                        flexDirection: "row",
                                        alignItems: "center",
